<template>
  <div>
    <div style="margin: 10px 0">
      <el-input style="width: 200px" placeholder="请输入戏曲名称" suffix-icon="el-icon-search" v-model="name"></el-input>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column label="审核" width="200" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="danger" slot="reference" @click="reviewFailed(scope.row.id)">不通过
          </el-button>
        </template>
      </el-table-column>
    </el-table>
          ...
          <el-upload ref="upload" action="" list-type="picture-card" :on-preview="handlePictureCardPreview"
            ...
            <template #tip>
              <div style="font-size: 12px;color: #919191;">
                单次限制上传一张图片,且只能上传jpg/png/jpeg文件
              </div>
            </template>
          </el-upload>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    Approved(id) {
      console.log(id);
      this.state = 1
      this.request.post(`/file/updateOperaByIdState?id=${id}&state=${this.state}&stateInfo=${this.stateInfo}`).then(res => {
    ...
    reviewFailedSave(stateInfo) {
      ...
      this.state = 2
      if (stateInfo !== '') {
        this.request.post(`/file/updateOperaByIdState?id=${this.id}&state=${this.state}&stateInfo=${stateInfo}`).then(res => {
          // console.log(res.data);
          if (res.data == 1) {
            this.$message.success('操作成功')
            this.stateInfo = ''
